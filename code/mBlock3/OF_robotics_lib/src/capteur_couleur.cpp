...
uint16_t Adafruit_TCS34725::calculateLux(uint16_t r, uint16_t g, uint16_t b)
{
  ...
  /* based exclusively on clear since 	 might be more reliable?      */
...
/**************************************************************************/
/*!
    Custom Functions to make it work with mBlock
*/
/**************************************************************************/

byte Adafruit_TCS34725::RedValue(byte gammatable[256]) {
...
byte Adafruit_TCS34725::GreenValue(byte gammatable[256]) {
	uint16_t clear, red, green, blue;
	this->getRawData(&red, &green, &blue, &clear);
	float g;
	uint32_t sum = clear;
...
byte Adafruit_TCS34725::BlueValue(byte gammatable[256]) {
	uint16_t clear, red, green, blue;
	this->getRawData(&red, &green, &blue, &clear);
	float b;
	uint32_t sum = clear;
